			...
			foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
				...
				if ( $_product->exists() && $values['quantity'] > 0 ) {
					...
					<tr class = "<?php echo esc_attr( apply_filters('woocommerce_cart_table_item_class', 'cart_table_item', $values, $cart_item_key ) ); ?>">
						...
						<!-- The thumbnail -->
						<td class="product-thumbnail product-name center-text">
							<?php
						...
						<td class="product-quantity">
							<?php
								if ( $_product->is_sold_individually() ) {
									...
									$product_quantity = sprintf( '<div class="quantity"><input type="number" name="cart[%s][qty]" step="%s" min="%s" max="%s" value="%s" size="4" title="' . _x( 'Qty', 'Product quantity input tooltip', 'woocommerce' ) . '" class="input-text qty text" maxlength="3" /></div>', $cart_item_key, $step, $min, $max, esc_attr( $values['quantity'] ) );
								}
